...
class Index:
    ...
    def calc_config(self, event):
        with open('NBS_GF_dict.pkl', 'rb') as fp:
            NBS_GF_dict = pickle.load(fp)
        #print(min(NBS_GF_dict.keys()))
        #print(NBS_GF_dict[min(NBS_GF_dict.keys())])
        print(NBS_GF_dict[min(NBS_GF_dict.keys())][0])
        #pass
        for i in NBS_GF_dict[min(NBS_GF_dict.keys())][0]:
            ax.plot([i.x],[i.y], marker = 'o', color ='black', markersize=12, alpha =0.5)
        plt.draw()
...
if __name__ == "__main__":
    ...
    axCalc_config = fig.add_axes(ax4)
    bCalc_config = Button(axCalc_config, 'Вычислить конфигурацию')
    bCalc_config.on_clicked(callback.calc_config)
